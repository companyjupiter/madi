...
- **언어 핀 (2026-08-31 백포트, §7 2차 프로브)**: lecture/interview는
  LIVE_SUMMARY의 프로브 확정 문안 "반드시 전사와 같은 언어로만 답하세요
  (전사가 영어면 영어로)"를 사용. **meeting과 condense 3종은 반증되어
  기존 문안 유지** (meeting 골든 바이트 동일 불변).
...
**2차 프로브 — 언어 핀 백포트 A/B (2026-08-31, LIVE_SUMMARY 확정 문안 이식):**

같은 하니스(엔진 2종 × KO/EN 픽스처, 프롬프트는 Swift 문안과 바이트 동일 생성,
final 12 + condense 9 케이스/모델). 후보 = "반드시 …과 같은 언어로만 답하세요
(…이 영어면 영어로)" (라이브 요약 4B 프로브에서 한글 누수 63→0을 만든 문안).

| 표면 | 판정 | 근거 (soft → 핀) |
|---|---|---|
| lecture final | **채택** | 2B EN 한글 424→**0**자(태그 제외, 건강한 영어 출력) · 2B KO 폭주 1959→575자 정상화 · 4B KO 바이트 동일 출력, 4B EN 무변화(무회귀) |
| interview final | **채택** | 2B EN 93→51자 + [요약]에 형식 명세 그대로 메아리치던 병리 소멸 · KO 2B/4B 무회귀 |
| meeting final | **반증 → 불변** | 4B KO가 '- 담당자:' 플레이스홀더를 리터럴 복사(액션 줄 형태 훼손) · 2B EN 354→1082자 폭주 + [결정] 태그 유실 |
| condense 3종 | **반증 → 불변** | 2B EN meeting 186→1085자·lecture 324→1156자 폭주 (interview 1픽스처만 개선 — 증거 부족으로 미채택) |

위치 변형(형식 명세 뒤 배치, 영어 힌트 병기)도 4B EN에서 전부 한국어 출력 —
4B의 EN 미개선은 문안 위치가 아니라 한국어 형식 스캐폴드("형식: [요약] 핵심을
2-4문장…")가 지배하는 구조적 한계. 골든 테스트:
`testProbedLanguageInstructionIsPinnedOnLectureAndInterview` (채택 2종 핀 +
반증 표면 무핀), meeting 바이트 동일 골든은 기존 그대로.

**알려진 한계 (백로그, 이 설계 스코프 외):**
- 영어 전사에서 [요약]이 한국어로 나오는 경향: lecture/interview는 2026-08-31
  언어 핀 백포트로 2B에서 해소(위 2차 프로브). **meeting은 핀이 반증되어 현행
  유지** — 4B는 세 템플릿 모두 EN 전사에 한국어 요약을 내는 구조적 한계 잔존
  (한국어 형식 스캐폴드 지배; 라이브 요약의 불릿-only 프롬프트와 달리 문안으로
  못 고침).
