...
| W-1 | **WER 표준 벤치 1차 측정** (resident STREAM 잡피드 + 공식 Whisper 정규화기 + jiwer) | ✅ data | **test-clean 2.17%** (2620발화/53k단어), **test-other 4.45%** (2939발화) — 공표 large-v3(2.0/3.9) 대비 turbo 열화 감안 시 기대대역. Q8+자체 Metal 파이프라인으로 레퍼런스급 절대 품질 입증. ⚠ 원본 jsonl은 /tmp 청소로 소실 — bench/wer_runs/(영구)에서 재현 중 |
| W-2 | **발화 게이트의 절대 진폭 민감성** — FLEURS-ko(피크 −33 dBFS 마스터링)에서 222/382 청크가 무음 스킵 | ✅ 진단 | 에너지 게이트(VAD_THRESH 0.010 > max-1s-RMS 0.004)와 **silero도 저게인에서 침묵** (VAD_THRESH=0으로도 silero 쪽 chunk_speech_s<0.25가 차단; +20dB 부스트 → 완벽 전사). 제품 함의: 저게인 마이크면 제품 전체 침묵 가능 → 캡처단 AGC 또는 게이트 전 정규화 후보. 벤치는 피크 정규화(-1 dBFS)로 해결 |
| W-3 | 엔진 wav 파서: IEEE float32 → 조용한 0샘플, 빈 파일 → segfault | ✅ 진단 | FLEURS가 float32 wav 출하 — 에러 없이 빈 전사. 별도 강건화 작업 분리(칩). 벤치는 pcm_s16le 강제 변환 |
