...
| W-2b | **게이트 전용 AGC 구현** (transcribe.zig) — 청크별 peak 정규화(타깃 0.3, 플로어 1.0, cap 32×) 스크래치 버퍼를 에너지RMS·silero·OSD에만 공급; **인코더는 원본 samples 불변**(레벨-robust, FLEURS CER 3.99% on raw). AGC=0 롤백 | ✅ commit | **저게인 FLEURS 빈값→완전전사**(peak 0.018~0.023 3파일 확인). 회귀: jfk 바이트동일, ES2004a DER **16.49% 동일**(무음청크 17× 부스트해도 silero가 비발화 기각), 제품경로 FLEURS-40 CER 3.11%. devops 타임스탬프 흔들림은 AGC 무관(AGC=0 2회도 자기들끼리 다름 = 기존 단어-끝 비결정성, 별도 백로그). 벤치도 게이트우회·외부정규화 핵 제거(제품 경로로 단순화) |
