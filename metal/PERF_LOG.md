...
|---|------|--------|--------|--------|
| BATCHDEC-M1 | **멀티청크 배치 디코드 in-engine 재통합(M1) — P5 배치 cross-attn 위에서** (BATCHDEC=1 게이트, 기본off 섀도우) | ✅ 텍스트 등가 + B=8 1.34× / ⚖️ 프로덕션 통합 보류(end-to-end ~타이) | P5(배치 cross-attn)로 J의 잔여원인(per-slot attn) 제거 후 **in-engine 섀도우 재통합**: 가중치 deq→WF16+tok_emb f16 1회, contiguous cross-KV, per-token(embed/pe per-slot→decodeBlockBatched→배치 LN+cvt+matmulF16Batched logit→per-slot suppress/filt/argmax→ragged EOT). **텍스트 등가 재확증**(devops_ko: batchdec B=4 `529 tok` == per-slot 149+144+113+123 = **529 정확 일치**, BPE 텍스트 동일). **속도(같은 실행 head-to-head)**: **B=4(기본 ENC_BATCH=4)=타이**(batched ~530 vs per-slot ~520 tok/s, +2%) / **B=8(ENC_BATCH=8)=1.34×**(batched ~660 vs per-slot ~500). **end-to-end 투영**: 디코드 23%(3.9s)→B=8서 ~2.95s, 절감 ~0.95s → 15.6→~14.6s **vs whisper.cpp 14.5s = 데드 타이**(잔여=로드 dequant, [[LOAD-mmap]]서 반증한 구조적 격차). **결론**: 디코드 tok/s는 B=8서 whisper.cpp 추월(660>595) — 방어가능 standalone claim, M1 섀도우로 입증완료. **M2(word-ts 배치 alignment 캡처)+M3(retire-to-seq) 프로덕션 재작성은 보류** — 헤드라인 end-to-end는 ~타이인데 전사경로(word-ts/rescue/seek) 회귀리스크 큼(ROI 부정). 스펙 "2×→13.7s 추월"은 순수 probe 2× 기준이었고 실파이프라인은 B=8-한정 1.34×→타이. quark atom `fn__decodeBlockBatched` | feat(decode) M1 |
