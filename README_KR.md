...
스레드 포화·번역 백로그·줄 수 비례 CPU·STT 루프 가드), **0.3.21**(2026-09-08, 빌드 742): 언어 게이트 P5/P6과 한글 전용 글로서리 프롬프트(P6-2), 메모리 M1–M3, 줄
...
**현재 `stable`: 0.4.1**(2026-09-20, `v0.4.1`, 빌드 781) — 게시된 유일한 버전. 라이선스와 고지를
바로잡는 릴리스다: Madi 자체 코드는 AGPL-3.0이고 라이선스 전문이 앱에 동봉되며, 서드파티 고지에
Sparkle과 DNA3.0-2B가 추가되고, 사전 빌드 번역 엔진은 누구나 무료로 쓰는 자체 바이너리 라이선스를
