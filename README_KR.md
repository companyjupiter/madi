...
> **상주 모델(resident)**: 기본적으로 `transcribe`를 **STREAM 모드**로 한 번만 띄워
> 모델(1.6GB)을 메모리에 상주시키고, 세그먼트를 FIFO로 흘려보내 **재로드 없이** 처리합니다
> (세그먼트당 ~1초 오버헤드 제거, cold-cache·짧은 세그먼트일수록 이득↑). 언어 감지도
> 첫 세그먼트에서 한 번만 → 이후 고정(언어 흔들림 방지). 문제 시 `--no-resident`로 폴백.
> bash 3.2 호환(coproc 대신 named pipe + fd 7/8). 화자 임베딩/클러스터링은 아직 가벼운
> 별도 프로세스로 처리합니다(추후 in-process 흡수 가능).
